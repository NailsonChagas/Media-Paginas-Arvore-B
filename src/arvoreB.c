...
static NodeB * split_pag(NodeB *pai, int posF_cheio){
    int i;
    NodeB *pag_esq = pai->filhos[posF_cheio]; // a página que será dividida: as (N - 1)/2 menores chaves permanecerão nessa página
    NodeB *pag_dir; // a nova página, onde serão colocadas as (N - 1)/2 maiores chaves, que estão na pag_esq. É importante ressaltar que, entre a nova chave e as que estão na ágina cheia, uma será promovida. Por isso, cada página deverá conter por volta de (N - 1)/2

    pag_dir = criar(); // nova página é gerada

    pag_dir->eh_no_folha = pag_esq->eh_no_folha; // se a pagina que está cheia é nó folha, então a nova página deverá ser folha também. Isso  vale tambm para as páginas não folha (nós internos)
    ...
    // se a página esquerda não for nó-folha, a página direita deve herdar os descendentes de suas respectivas chaves
    ...
    // A partir daqui, é atualizada a página pai, para que na mesma seja adicionada a chave que foi promovida
    // Os descendentes da página pai são deslocados em uma posição para a adição da nova página descendente
    // A página esquerda permanecerá no mesmo lugar. Como uma chave foi promovida, então um ponteiro deve apontar para a pag_dir. Esse ponteiro ficará na posição posF_cheio + 1
    ...
    // Anteriormente a posição dos filhos da página pai foi atualizada. Agora, as chaves da página pai terão a suas respectivas posições atualizadas para alocar a chave promovida
    for (i = 0; i < pag_dir->nro_chaves ; i++)
...
Caso contrário, um teste adicional deve ser feito: se página não folha não estiver cheia, a inserção pode ser feita sem problemas em uma página descendente (chamada recursiva dessa função).
Se página não folha estiver cheia, a mesma deve ser divida utilizando a função split_pag e uma nova tentativa de inserção (chamada recursiva dessa função) deve ser efetuada.
...
static NodeB * inserir_pagina_nao_cheia(NodeB *tree, int key){
    int i;
    int pos = busca_binaria(key, tree); //encontrar a posição para adicionar a chave ou a posição de uma outra página para continuar a busca

    // Verificar se uma página é folha. Caso positivo, é presumido que a chave não esteja cheia.
    // CASO BASE
    if (tree->eh_no_folha){
        // Um deslocamento de chaves da página pode ser realizado para colocar a nova chave em sua posição correta
        for (i = tree->nro_chaves; i > pos; i--)
            tree->chaves[i] = tree->chaves[i - 1];

        // ṕosicionamento da nova chave na página
        tree->chaves[i] = key;
    ...
    // CASO ITERATIVO
        ...
        // Se a página analisada estiver cheia, a mesma deve ser dividida previamente. Por mais que a nova chave não seja inserida nessa página, uma outra chave será promovida
        if (tree->filhos[pos]->nro_chaves == N - 1){
            tree = split_pag(tree, pos);

            // Como pos é resultante da busca binária implementada neste TAD, o resultado deve ser a posição da chave ou a posição de uma página filha, onde pode ser efetuada uma nova busca.
            // Após a divisão de uma página filha, a página pai será atualizada. Nessa operação, "pos" poderá ficar obsoleta, pois uma chave menor poderá ocupar essa posição na página. Por isso, "pos" deve ser incrementada para indicarmos a página onde será feita a próxima busca (chamada recursiva).
...
NodeB * inserir(NodeB *tree, int key){
    ...
    // Um detalhe omitido em aula, é que durante a inserção, se a raiz estiver cheia, a mesma deve ser repartida. Quando isso ocorre, a altura da árvore aumenta
    if (aux->nro_chaves == N - 1){
        ...
        // A nova página passa a ser raiz, onde o lado esquerdo do seu primeiro elemento (a ser promovido) será a que era raiz, a qual será dividida
        nova_pag->filhos[0] = aux;

        // a página aux será dividida
        nova_pag = split_pag(nova_pag, 0);

        // Por fim,  feita a tentativa de inserção da nova chave na árvore, na qual será procurada uma página folha
        nova_pag = inserir_pagina_nao_cheia(nova_pag, key);

        // atualização da raiz
        tree = nova_pag;
    }else
    // Se a raiz não estiver cheia, a inserção  feita em uma das páginas folhas, as quais podem ser divididas se as mesmas estiverem cheias
...
int media_paginas_folhas(NodeB* root){
    int i=0,j=0, k, l = 0, soma=0, cont = 0, media;
    NodeB *auxA = root, *auxB = NULL;

    if(root->eh_no_folha == 1){
        for(k=0; k<root->nro_chaves; k++){
               soma+=root->chaves[k];
               cont++;
        }
    }
    while(root->eh_no_folha != 1){
        auxB = root;      
        root = root->filhos[i];
        ...
        if(root->eh_no_folha == 1 && i < auxB->nro_chaves){
            root = auxB;
            i++;
        ...
        else if(root->eh_no_folha == 1 && i >= auxB->nro_chaves && j<auxA->nro_chaves){
            root = auxA;
            j++;
            i = j;
            l = 1;
